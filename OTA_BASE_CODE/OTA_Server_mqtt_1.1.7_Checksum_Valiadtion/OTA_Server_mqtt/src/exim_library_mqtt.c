...
void exim_on_connect(struct mosquitto *mosq, void *userdata, int rc) {
    if (rc == 0) {
        // Connection successful, subscribe to both topics separately
        if (exim_mosquitto_subscribe(mosq, NULL, TOPIC_DEVICE_VERSION, 0) == MOSQ_ERR_SUCCESS &&
            exim_mosquitto_subscribe(mosq, NULL, TOPIC_DEVICE_ACK, 0) == MOSQ_ERR_SUCCESS) {
            logger("INFO", "mosquitto_subscribe: Success\n");
        } else {
            logger("ERROR", "mosquitto_subscribe: Failed\n");
        }   
    } else {
