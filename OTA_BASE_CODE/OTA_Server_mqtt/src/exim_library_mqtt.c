    ...
    if (message->payloadlen > 0) {
        ...
        if (strcmp(message->topic, TOPIC_DEVICE_VERSION) == 0) {
            ...
            if (sscanf((char *)message->payload, "%255[^:]:%255s", device_id, version) == 2) {
                //printf("Device ID: %s, Version: %s\n", device_id, version);//when used logger following below statements are not executed
                if (is_device_selected(device_id)) {
                    send_file_in_bytes(mosq, file_path, device_id); // Send bin file
                } else {
                    printf("Device ID %s is not in the selected list. Ignoring file transfer.\n", device_id);//when used logger facing issue
                }
            } else {
                printf("Failed to parse version information from payload.\n"); //- when used logger facing issue
            }
        }

        // Handle acknowledgment from the subscriber(device)
        else if (strncmp(message->topic, TOPIC_DEVICE_ACK, strlen(TOPIC_DEVICE_ACK)) == 0) {
            //printf("Acknowledgment received: %s\n", (char *)message->payload);//when used logger following acknowledgment is not coming from device
        }
    }
}
...
int exim_mosquitto_publish(struct mosquitto *mosq, int *mid, const char *topic, int payloadlen, const void *payload, int qos, bool retain) {
    if (mosq == NULL || topic == NULL || payload == NULL) {
        logger("ERROR", "exim_mosquitto_publish: Invalid parameter(s), mosq, topic, or payload is NULL.\n");
        return MOSQ_ERR_INVAL;
    }
